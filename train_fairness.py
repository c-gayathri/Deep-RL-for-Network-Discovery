...
# --sample-budget   | Budget for no. of queries |       --- 5 ---
...
def arg_parse():
    ...
    parser.add_argument('--fairness',dest='fairness', type=str,default=1,
    ...
    parser.add_argument('--fileName', dest='fileName', type=str,default='results/raw/fairness_experiment.txt',
...
with open(gparams_path, 'rb') as handle:
    ...
    print('--------------------- CODE PRINT ----------------------------\n\n\n\n\n')
    print('DEBUG: comSizes - ', comSizes) # Debug
    print('\n\n\n\n\n--------------------- PRINT END ----------------------------')
    node_community = graph_params['node_community']
    g = graph_params['graph']
    p_in = graph_params['p_in']
    p_out = graph_params['p_out']
graphs = [g]
...
for g,seeds in zip(graphs, e_seeds_list): # e_seeds_list: For each graph in 'graphs', get 'extra_seeds' no. of random seed nodes => a list of lists
    env = NetworkEnv(fullGraph=g, fairness = fairness, seeds=seeds, alpha = alpha, comSizes=comSizes, node_community=node_community, opt_reward=0, nop_r=args.nop_reward,
...
if __name__ == "__main__":
    ...
    try:
        ...
        logging.info(
            'Parameters: fairness=%s budget=%s extra_seeds=%s alpha=%s '
            'factor=%s samples=%s graph_size=%s p_in=%s p_out=%s',
            fairness,
            budget,
            extra_seeds,
            alpha,
        ...
        for ep in range(NUM_EP):
            ...
            outputs = [str(epTime), str(inf), str(discBand), str(sprBand), str(minDisc), str(minSpr), str(comDisc), str(comSpread)]

            with open(file_name, 'a') as file:
                csv.writer(file).writerow(outputs)
