# Calculates both influence spread and commuity discovery
# Based on a boolean value, returns reward as either influence spread or community discovery
# Doesn't allow normalise or clipping
# Here, the values are divided by total size of the network and nor optimum

# When no fairness, reward (which is equal to the influence) is normalised by length (no. of vertices) of full graph
# Instead of normalising by the optimal value (greedy algo when full graph is known)

import numpy as np
import networkx as nx
from copy import copy
from expts.influence import influence, parallel_influence, genoptfunction, comobjfunction
...
class NetworkEnv(object):
    ...
    def __init__(self,fullGraph, fairness = False, alpha = 1, node_community = [], comSizes = [], seeds=[], max_T=100, influence_algo=influence, opt_reward = None, nop_r=0, times_mean=1, bad_reward=0, normalize = False, clip_min = None, clip_max = None):
    ...
    def step(self, action):
        ...
        if action == -1:
            ...
            # CHANGE
            # ----------------------------------------------------------------------------------------
            self.influence_ = parallel_influence(self.graph, full_graph=self.fullGraph,
                                influence=self.influence_algo, times=self.times_mean)
            self.influence = self.influence_/len(self.fullGraph)

            # Returns a list containing the influence spread in each community
            self.comobj_ = comobjfunction(self.fullGraph, self.active, self.node_community, len(self.comSizes))
            self.comobj = [nodes/self.comSizes[i] for i, nodes in enumerate(self.comobj_)]
            print('\n\t\t\t\tDEBUG\n\n\n\n')
            print('len(self.fullGraph): ', len(self.fullGraph))
            print('len(self.active): ', len(self.active))
            print('len(self.node_community): ', len(self.node_community))
            print('self.comobj_: ', self.comobj_)
            print('self.comSizes:', self.comSizes)
            print('Check')
            print('\n\n\n')

            if self.fairness == True:
                welf = 0
                for c in range(len(self.comSizes)):
                    welf += math.pow(self.comobj[c]/self.comSizes[c], self.alpha)/self.alpha
                self.reward_ = welf
            else:
                self.reward_ = welf = self.influence
            ...
            return self.graph, self.reward, self.done, self.influence, self.comobj
        ...
        if action in self.possible_actions:
            ...
            if self.T > self.max_T:
                self.done = True
            return self.graph, self.reward, self.done, None, None
