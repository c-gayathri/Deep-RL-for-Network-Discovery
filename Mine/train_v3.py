...
# ----------------------------------------------------------------------------------------
ratios = [0.4,0.4,0.2]
communities = count = comSpread = [0]*len(ratios)
alpha = 0.5
comNodes = [[]]*len(ratios)
# ----------------------------------------------------------------------------------------
...
print('--------------------- CODE PRINT ----------------------------\n\n\n\n\n')
    ...
    # CHANGE
    # ----------------------------------------------------------------------------------------
g = nx.read_gpickle("sbm.gpickle")
with open('graph_params.pickle', 'rb') as handle:
    ...
    communities = graph_params['communities']
...
for g,seeds in zip(graphs, e_seeds_list): # e_seeds_list: For each graph in 'graphs', get 'extra_seeds' no. of random seed nodes => a list of lists
    env = NetworkEnv(fullGraph=g, seeds=seeds, alpha = alpha, communities_boo=1, communities=communities, node_community=node_community, opt_reward=0, nop_r=args.nop_reward,
    ...
    try:
        for ep in range(NUM_EP):
            ...
            for stps in range(budget):
                ...
                print('\n\t\t\t\tDEBUG\n\n\n\n')
                print('list(env.possible_actions)')
                print(list(env.possible_actions))
                ...
                count[c] += 1
                comSpread[c] = count[c]/communities[c]
                welf = 0
                for u in comSpread:
            ...
            for c in len(ratios):
                comSpread = f_set(set(comNodes(c)))/communities(c)'''
