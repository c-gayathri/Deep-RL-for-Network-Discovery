...
class Change:
    ...
    def enlarge_graph(self,u): # Add node u
    ...
    def sample_graph1(self): 
        self.v1 = random.sample(range(len(self.graph)),int(self.budget/2)) # sample 'budget/2' items from [0,1,2...(len-1)]
        # Choose budget/2 nodes from the graph
        for u in self.v1:
    ...
    def sample_graph2(self):
        ...
        # For every node 'u' in v1 + seeds, choose a random node 'v' from its neighbours and add its edges. Add 'v' to v2
        for u in self.v1:
