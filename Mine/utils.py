...
def greedy(items, budget, f, init = []):
    ...
    '''
    # Starts with zero items; ranks items with items with highest function values at the start; draw from this heap, add to set and evaluate marginal gain to function
    # if marginal gain is not higher than function value of the item added - 0.01 (small epsilon) then check next item
    # QUESTION: what's the point of ranking if we aren't assured that that's the best? And why epsilon?
    import heapq
    upper_bounds = [(-f(set([u])), u) for u in items] # assuming the function f gives upper bound    
    heapq.heapify(upper_bounds) # sorts by the first element of the tuple => highest upper bounds listed first
    starting_objective = f(set(init))
    S  = set(init)
    #greedy selection of K nodes
    while len(S) < budget:
        val, u = heapq.heappop(upper_bounds) # return smallest element (here, highest upper bound)
        ...
        if new_val >= -upper_bounds[0][0] - 0.01:
            ...
            heapq.heappush(upper_bounds, (-new_val, u))
    return S, starting_objective # Set of nodes used, objective function value
