# Regular IM
# Smaller graph
# Measuring band size and influence spread
...
gparams_path = 'graph_params_400_single.pickle'
...
def arg_parse():
    ...
    parser.add_argument('--fileName', dest='fileName', type=str,default='epsData_v4_reg.txt',
...
for g,seeds in zip(graphs, e_seeds_list): # e_seeds_list: For each graph in 'graphs', get 'extra_seeds' no. of random seed nodes => a list of lists
    env = NetworkEnv(fullGraph=g, seeds=seeds, opt_reward=0, fairness = False, nop_r=args.nop_reward, node_community=node_community, comSizes = comSizes,
        ...
        for ep in range(NUM_EP):
            ...
            for stps in range(budget):
                ...
                if stps == budget-1 or len(env.possible_actions)==0:
                    ...
                    '''debugCom = comobjfunction(g, env.active, node_community, len(comSizes))
                    print('\n\t\t\t\tDEBUG\n\n\n\n')
                    print('g == env.fullGraph: ', g == env.fullGraph)
                    #print('g == env.fullGraph: ', g == env.fullGraph)
                    print('node_community == env.node_community: ', node_community == env.node_community)
                    print('len(comSizes) == len(env.comSizes): ', len(comSizes) == len(env.comSizes))
                    print(env.comSizes)
                    print('debugCom: ', debugCom)
                    print('Check\n\n')'''
                    # ----------------------------------------------------------------------------------------
