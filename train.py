# v6: og but measuring influence | uses net_env_v2

#GCNQ: Multi Q
import numpy as np
import networkx as nx
import random, io #dependecy
# folder
...
# folder
from expts.influence import influence # returns 1) influence over full graph 2) over the current discovered graph & 3) the choice of greedily chosen influential nodes
import pickle, os
# folder
...
gparams_path = 'data/graphs/graph_extreme.pickle'
...
def arg_parse():
    ...
    parser.add_argument('--newFile',dest='newFile', type=int,default=0, choices=[0, 1],
                    help='whether to create a newFile')
    parser.add_argument('--fileName', dest='fileName', type=str,default='results/raw/experiment.txt',
...
infl.SAMPLES = args.samples # '--samples',dest='samples', type=int, default=100, help='Number of samples in Influence Maximization'
...
def get_action(s, emb,nodes):
    ...
    node = -1
    for v in nodes:
        value, _ = acmodel.get_values2_(s[0], s[1], emb[v]) # ~ self.actor_critic.critic_forward
        if value>q_vals:
            q_vals = value
            node = v
    return node, q_vals
